...
Herkömmliche Investitionsrechnungen in der Ölindustrie verwenden typischerweise einen festen Ölpreis, feste Fördermengen und feste Kostenansätze. Diese deterministischen Modelle liefern einen einzigen ROI-Wert, der suggeriert: \glqq Das Projekt rendiert X\%\grqq{} Diese Aussage ist jedoch irreführend, weil sie drei kritische Aspekte ignoriert:
...
Das Problem: Wenn jede Variable eine Wahrscheinlichkeitsverteilung besitzt, wird der ROI selbst zu einer Zufallsvariablen. Der \glqq ROI\grqq{} ist dann keine Zahl mehr, sondern eine Verteilung — und die Form dieser Verteilung enthält die entscheidenden Informationen über das Risiko.
